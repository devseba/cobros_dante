<div class="pad">
	<h2>Listado de deudas <span>
		<a href="<?=site_url('deudas/exportTodo')?>">
			<img src="<?=site_url('static/img/icon/doc_export.png')?>">
		</a>
		</span></h2>
	<div id="filtros">
		<form action="" method="post" id="f">
			<?=img('static/img/icon/calendar_1.png')?>
			<?=form_input('fecha_desde',isset($filtros['fecha_desde'])?mdate('%d/%m/%Y' ,mysql_to_unix($filtros['fecha_desde'])):date('d/m/Y', mktime(0,0,0,1,1,"2000")),'id="fecha_desde" class="date tipns" title="Fecha Inicial" disabled="disabled"')?>
			<?=img('static/img/icon/calendar_1.png')?>
			<?=form_input('fecha_hasta',date('d/m/Y',mktime(0,0,0,12,31,date('Y'))),'id="fecha_hasta" class="date tipns" title="Fecha Final"')?>
