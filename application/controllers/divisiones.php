...
class Divisiones extends CI_Controller {
	...
	public function index($offset = 0){
		if(!$offset)
			$this->session->unset_userdata('filtros_divisiones');
			
		$datos = $this->session->all_userdata();
		
		$string = isset($datos['filtros_divisiones']['division'])?$datos['filtros_divisiones']['division']:'%%';
	...
	public function filters($offset=0){
		...
		$this->session->set_userdata('filtros_divisiones', $valores);
	...
	function eliminar($id){
		try{
			...
			$this->session->set_flashdata('msg','<div class="error">La División no se puede eliminar.</div>');
			}
		redirect('divisiones');
	}
}
