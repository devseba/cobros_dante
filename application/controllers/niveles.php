...
class Niveles extends CI_Controller {
	...
	public function index($offset = 0){
		...
		$string = isset($datos['filtros_niveles']['nivel'])?str_replace(' ','%',$datos['filtros_niveles']['nivel']):'%%'; 
		...
		foreach($niveles as $nivel)
		{
			$this->table->add_row(
				$nivel->id,
				$nivel->nivel,
				($nivel->id==1)?'':anchor('niveles/editar/'.$nivel->id,img('static/img/icon/pencil.png'), 'class="tipwe" title="Editar"').' '.
	...
	public function filters($offset=0)
	{
		...
		$this->session->set_userdata('filtros_niveles', $valores);
		$conditions = array_merge(array($condiciones), $valores);
