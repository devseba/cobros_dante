...
<div class="yui-gb">
	...
	<div class="yui-u first">
		<div class="pad forms">
			...
			<? $gs = array('NO' => 'NO', 'A+' => 'A+', 'B+' => 'B+', 'A-' => 'A-', 'AB+' => 'AB+', 'AB-' => 'AB-', 'O+' => 'O+', 'O-' => 'O-' );?>
			...
			<?php echo isset($errors) ? $errors->on('lugar_nacimiento') : ''; ?>
			<?=form_input('lugar_nacimiento', isset($a->lugar_nacimiento) ? $a->lugar_nacimiento : set_value('lugar_nacimiento'),'id="lugar_nacimiento" tabindex=16')?>
			<?=form_label('Observaciones médicas','obs_medicas')?>
			<?php echo isset($errors) ? $errors->on('obs_medicas') : ''; ?>
			<?=form_textarea('obs_medicas',isset($a->obs_medicas) ? $a->obs_medicas : set_value('obs_medicas'), 'id="obs_medicas" tabindex=19')?>
		</div>
	</div>
	<div class="yui-u">
		<div class="pad forms">
			...
			<?=form_input('colegio_procedencia', isset($a->colegio_procedencia) ? $a->colegio_procedencia : set_value('colegio_procedencia'),'id="colegio_procedencia" tabindex=17')?>
			<?=form_label('Observaciones generales','observaciones')?>
			<?php echo isset($errors) ? $errors->on('observaciones') : ''; ?>
			<?=form_textarea('observaciones',isset($a->observaciones) ? $a->observaciones : set_value('observaciones'), 'id="observaciones" tabindex=20')?>
		</div>
	</div>
	<div class="yui-u">
		<div class="pad forms">
			...
			<?=form_input('fecha_inscripcion', isset($a->fecha_inscripcion) ? $a->fecha_inscripcion->format('d/m/Y') : date('d/m/Y'),'id="fecha_inscripcion" class="date" tabindex=18')?>
		</div>
	</div>
</div>
<div class="pad">
	<?=form_submit('enviar','Guardar','tabindex=21')?>
	<?
